...
export default function ApplyPage() {
    ...
    useEffect(() => {
        if (!token) {
            router.push('/auth/login?redirect=/listener/apply');
        }
    }, [token, router]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await api.post('/listeners/apply', {
                bio: formData.bio,
            ...
            <main className="bg-dark min-h-screen pt-32">
                ...
                <div className="flex-1 container mx-auto px-6 py-32 max-w-2xl text-center">
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-card p-12 md:p-20 border-white/5">
                        ...
                        <button onClick={() => router.push('/')} className="px-10 py-4 bg-white/5 border border-white/10 text-white rounded-xl font-black uppercase tracking-widest hover:bg-white/10 transition-all">
                            Back to Home
                        </button>
                    </motion.div>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    ...
                                    <input required disabled type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white opacity-50" value={user?.name || ""} />
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-white/40 uppercase tracking-widest px-1">Email Address</label>
                                    <input required disabled type="email" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white opacity-50" value={user?.email || ""} />
                                </div>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-white/40 uppercase tracking-widest px-1">How many years of life experience or listening experience do you have?</label>
                                <input required type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/10 focus:outline-none focus:border-primary/50" placeholder="e.g. 5 years, Life experience" value={formData.experience} onChange={e => setFormData({...formData, experience: e.target.value})} />
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-white/40 uppercase tracking-widest px-1">Languages you speak (comma separated)</label>
                                <input required type="text" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/10 focus:outline-none focus:border-primary/50" placeholder="English, Hindi, etc." value={formData.languages} onChange={e => setFormData({...formData, languages: e.target.value})} />
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-white/40 uppercase tracking-widest px-1">Why do you want to listen?</label>
                                <textarea required rows={4} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/10 focus:outline-none focus:border-primary/50 resize-none" placeholder="We value genuine empathy. Tell us why you're a good fit." value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} />
                            </div>
                            ...
                            <button type="submit" disabled={loading} className="w-full py-6 bg-linear-to-r from-primary to-accent text-white font-black uppercase tracking-widest rounded-xl shadow-xl shadow-primary/20 hover:shadow-primary/40 transform hover:-translate-y-1 transition-all flex items-center justify-center gap-2">
